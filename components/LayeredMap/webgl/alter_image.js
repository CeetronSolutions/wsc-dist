...
function alter_image(canvas, map_base64, colormap_base64, hillshading, elevation_scale, light_direction) {
    ...
    const load_image = (src) => new Promise((resolve) => {
        const img = new Image();
        img.src = src;
        img.onload = () => resolve(img);
    });
    Promise.all([load_image(map_base64), load_image(colormap_base64)]).then(function ([map_image, colormap_image]) {
        ...
        const program = createProgram(gl, createShader(gl, gl.VERTEX_SHADER, vertexShaderSource), createShader(gl, gl.FRAGMENT_SHADER, hillshading
        ...
        gl.uniform2f(gl.getUniformLocation(program, "u_resolution_vertex"), gl.canvas.width, gl.canvas.height);
        gl.uniform1f(gl.getUniformLocation(program, "u_colormap_length"), colormap_image.width);
        if (hillshading) {
            gl.uniform2f(gl.getUniformLocation(program, "u_resolution_fragment"), gl.canvas.width, gl.canvas.height);
            const vectorLength = Math.sqrt(Math.pow(light_direction[0], 2) +
                Math.pow(light_direction[1], 2) +
                Math.pow(light_direction[2], 2));
            gl.uniform3f(gl.getUniformLocation(program, "u_light_direction"), light_direction[0] / vectorLength, light_direction[1] / vectorLength, light_direction[2] / vectorLength);
            gl.uniform1f(gl.getUniformLocation(program, "u_elevation_scale"), elevation_scale);
        }
...
//# sourceMappingURL=alter_image.js.map