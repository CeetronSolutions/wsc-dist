...
export const makePolyline = (item, swapXY, setProps) => {
    ...
    const shape = addTooltip(item, L.polyline(pos, {
...
export const makePolygon = (item, swapXY, setProps) => {
    const pos = swapXY ? item.positions.map((xy) => yx(xy)) : item.positions;
    const shape = addTooltip(item, L.polygon(pos, {
        color: item.color || "blue",
        positions: pos,
    }));
    shape.on("mouseup", () => {
...
export const makeCircle = (item, swapXY, setProps) => {
    ...
    const shape = addTooltip(item, L.circle(center, {
...
export const makeCircleMarker = (item, swapXY) => {
    const center = swapXY ? yx(item.center) : item.center;
    return addTooltip(item, L.circleMarker(center, {
        color: item.color || "red",
        center: center,
...
export const addImage = (imageData) => {
    ...
    const newImageLayer = L.imageWebGLOverlay(imageData.url, bounds, Object.assign(Object.assign({}, imageData), { minvalue: imageData.minvalue, maxvalue: imageData.maxvalue, colorScale: imageData.colorScale, shader: imageData.shader }));
    return newImageLayer;
};
export const addTile = (tileData) => {
    const newTileLayer = L.tileWebGLLayer(tileData.url, Object.assign(Object.assign({}, tileData), { minvalue: tileData.minvalue, maxvalue: tileData.maxvalue, colorScale: tileData.colorScale, shader: tileData.shader }));
    return newTileLayer;
};
