...
class D3PriorPosterior {
    ...
    createXAndColorScale() {
         ...
         */
        this.global_min = Math.min(...this.data.values.map((values) => Math.min(...values)));
        this.global_max = Math.max(...this.data.values.map((values) => Math.max(...values)));
        this.x_scale = d3
            .scaleLinear()
    ...
    updateTooltipLocation(x_value) {
        const normalized_x = (x_value - this.global_min) / (this.global_max - this.global_min);
        const tooltip_width = this.tooltip.node().getBoundingClientRect().width;
        const mouse_position = d3.mouse(this.svg.node());
        this.tooltip
            .style("left", mouse_position[0] - tooltip_width * normalized_x + "px")
    ...
    calculateBins() {
        ...
        */
        const histogram = d3
            ...
            .thresholds(d3.range(this.global_min, this.global_max, (this.global_max - this.global_min) / this.number_bins));
        this.bins = this.data.values.map((values) => histogram(values));
        const number_data_points = this.data.values.map((values) => values.length);
        this.bins.map((bins, iteration_index) => bins.map((bin) => (bin.percent =
            (bin.length * 100) /
                number_data_points[iteration_index])));
        const max_percent = Math.max(...this.bins.map((bins) => Math.max(...bins.map((bin) => bin.percent))));
        this.y_scale = d3
            .scaleLinear()
    ...
    renderHistogram() {
        ...
        this.histogram_g
            ...
            .attr("height", (d) => this.height_histogram - this.y_scale(d.percent))
            .style("fill", (d) => this.color_scale(0.5 * (d.x1 + d.x0)))
            .on("mouseover", (_, i) => {
            this.tooltip
                .style("opacity", 1)
                .text(this.bins[this.iteration_index][i].percent.toPrecision(3) + " %");
        })
            .on("mousemove", (d) => {
    ...
    updateHistogram() {
        this.histogram_g
            ...
            .transition()
            .duration(250)
            .attr("y", (_, i) => this.y_scale(this.bins[this.iteration_index][i].percent))
            .attr("height", (_, i) => this.height_histogram -
            this.y_scale(this.bins[this.iteration_index][i].percent));
    }
    renderPileChart() {
        const temp_nodes = {};
        this.data.values.forEach((values, iter_index) => values.forEach((value, sample_index) => {
        ...
        const tick = () => {
            d3.selectAll(".prior_posterior_circle")
                .attr("cx", (d) => d.x)
                .attr("cy", (d) => d.y)
                .attr("fill", (d) => this.color_scale(d.values[this.iteration_index]));
        };
        ...
        this.pile_chart_circles = this.pile_chart_g
            ...
            .on("mousemove", (d) => this.updateTooltipLocation(d.values[this.iteration_index]))
            .on("mouseout", () => this.tooltip.style("opacity", 0));
        this.pile_chart_circles
            .attr("r", (d) => this.iteration_index in d.values ? this.radius_circles : 0)
            .attr("visibility", (d) => this.iteration_index in d.values ? "visible" : "hidden");
        this.simulation = d3
            .forceSimulation(data)
            .force("x", d3.forceX((d) => this.iteration_index in d.values
...
//# sourceMappingURL=prior_posterior_distribution.js.map