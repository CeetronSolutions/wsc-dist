 ...
 * sqrt - Square root
 */
const functionWhitelist = ["ln", "log10", "sqrt"];
...
const parseExpression = (expression) => {
    ...
    // Filter duplicates
    const functions = functionNodes.filter((elm, idx) => functionNodes.indexOf(elm) === idx);
    const symbols = symbolNodes.filter((elm, idx) => symbolNodes.indexOf(elm) === idx);
    // Variables are the symbols not present among functions
    const variables = symbols.filter((sym) => !functions.some((func) => func === sym));
    const mulCharVars = variables.filter((elm) => elm.length > 1);
    if (mulCharVars.length > 0) {
        throw new Error("Not allowed with multi character variables: " + mulCharVars);
    }
    return { variables: variables };
};
export const getExpressionParseData = (expression) => {
    try {
