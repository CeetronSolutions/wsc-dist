...
import { computeAllowedAttributeValues, 
// eslint-disable-next-line prettier/prettier
extractAttributesTree } from "../../utils/dataUtil";
 ...
 */
const WellAttributesSelector = React.memo(() => {
    // Style
    ...
    const filterByAttributes = useSelector((st) => st.ui.filterByAttributes);
    const wells = useMemo(() => data.wells, [data]);
    //Create the tree that the SmartNodeSelector can accept as input.
    const attributesTree = useMemo(() => extractAttributesTree(wells, attributeKeys), [wells]);
    ...
    const hintText = useMemo(() => {
        ...
        return ("Well selection criteria: " +
    ...
    // Handlers
    const handleSelectionChange = useCallback((selection) => dispatch(updateFilterByAttributes(selection.selectedNodes)), [dispatch]);
    // Render
    return (React.createElement("div", { className: classes.root },
        React.createElement(SmartNodeSelector, { id: "AttributesSelector", key: "attributes-selector", numMetaNodes: 0, delimiter: ":", selectedTags: filterByAttributes, setProps: handleSelectionChange, label: "Filter by Attributes", data: attributesTree, numSecondsUntilSuggestionsAreShown: 0.5 }),
        React.createElement(Typography, null, hintText)));
});
WellAttributesSelector.displayName = "WellAttributesSelector";
export default WellAttributesSelector;
//# sourceMappingURL=WellAttributesSelector.js.map