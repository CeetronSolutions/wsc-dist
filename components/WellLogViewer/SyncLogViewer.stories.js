...
const ComponentCode = '<SyncLogViewer id="SyncLogViewer" \r\n' +
    "    syncTrackPos==true \r\n" +
...
export default {
    ...
    argTypes: Object.assign(Object.assign({}, argTypesSyncLogViewerProp), { 
        /*id: {
            description:
        ...
        */
        horizontal: {
            ...
            description: "Options for well log view:<br/>" +
...
const Template = (args) => {
    ...
    const setInfo = function (info) {
        if (infoRef.current)
            infoRef.current.innerHTML = info;
    };
    const [controller, setController] = React.useState(null); // the first WellLog
    const onCreateController = React.useCallback((iView, controller) => {
        if (iView === 0)
            setController(controller);
    }, [controller]);
    const onContentRescale = React.useCallback((iView) => {
        if (iView === 0)
            setInfo(fillInfo(controller));
    }, [controller]);
    const onContentSelection = React.useCallback(( /*_iView*/) => {
        /*if(iView===0)*/ setInfo(fillInfo(controller));
    }, [controller]);
    return (React.createElement("div", { style: { height: "92vh", display: "flex", flexDirection: "column" } },
        React.createElement("div", { style: { width: "100%", height: "100%", flex: 1 } },
            React.createElement(SyncLogViewer, Object.assign({ id: "SyncLogViewer" }, args, { onCreateController: onCreateController, onContentRescale: onContentRescale, onContentSelection: onContentSelection }))),
        React.createElement("div", { ref: infoRef, style: { width: "100%", flex: 0 } })));
};
const patternImages = [
...
//# sourceMappingURL=SyncLogViewer.stories.js.map