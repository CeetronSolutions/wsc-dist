...
let __idGradient = 0;
function createGradient(g, colorTable, rLogarithmic) {
 ...
 */
export default function renderGradientFillPlotLegend(g, bounds, legendInfo, plot) {
    ...
    if (options.inverseColor || options.inverseColorTable) {
        ...
        if (colorTable) {
            const id = createGradient(g, colorTable, options.scale === "linear" && options.colorScale === "log"
                ? max / min
                : undefined);
            fillNrm = "url(#" + id + ")";
        }
        colorTable =
            useMinAsBase && minIsLeft
                ...
                : options.colorTable;
        if (colorTable) {
            const id = createGradient(g, colorTable, options.scale === "linear" &&
    ...
    renderBasicPlotLegend(g, bounds, legendInfo.label ? legendInfo.label : "", legendInfo.unit ? legendInfo.unit : "", plot.scale.domain(), plot.options.color ? plot.options.color : "", true);
}
//# sourceMappingURL=gradientfill-plot-legend.js.map