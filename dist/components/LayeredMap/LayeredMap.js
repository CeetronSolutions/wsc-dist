...
import { LayersControl, Map, ScaleControl, FeatureGroup, CircleMarker, } from "react-leaflet";
...
class LayeredMap extends Component {
    ...
    resetView() {
        ...
        const initial_zoom = Math.min(Math.log2(height / (ymax - ymin)), Math.log2(width / (xmax - xmin)));
    ...
    setEvents() {
        this.mapRef.current.leafletElement.on("zoomanim", (ev) => {
            this.props.sync_ids
                ...
                .map((id) => {
                if (_layeredmap_references[id].mapRef.current.leafletElement.getZoom() !== ev.zoom) {
                    _layeredmap_references[id].mapRef.current.leafletElement.setView(ev.center, ev.zoom);
                }
            });
        });
        ...
        this.mapRef.current.leafletElement.on("click", (ev) => {
            ...
            this.props.sync_ids.map((id) => {
                _layeredmap_references[id].updateCircleMarkerPosition(ev.latlng.lng, ev.latlng.lat);
            });
        });
        this.mapRef.current.leafletElement.on("move", (ev) => {
            this.props.sync_ids
                ...
                if (typeof ev.originalEvent !== "undefined") {
                    _layeredmap_references[id].mapRef.current.leafletElement.setView(ev.target.getCenter());
                }
            });
        });
    }
    ...
    render() {
        ...
        const showDrawControls = draw_toolbar_marker || draw_toolbar_polygon || draw_toolbar_polyline
            ? true
            : false;
        ...
        const renderBaseLayer = (layer, key) => (React.createElement(CompositeMapLayer, { layer: layer, key: key, hillShading: this.state.hillShading, lightDirection: this.props.lightDirection }));
        const renderOverlay = (layer, key) => (React.createElement(CompositeMapLayer, { layer: layer, key: key, hillShading: this.state.hillShading, lightDirection: this.props.lightDirection, lineCoords: (coords) => setProps({ polyline_points: coords }), polygonCoords: (coords) => setProps({ polygon_points: coords }) }));
        return (React.createElement(Map, { id: this.props.id, style: { zIndex: 0, height: this.props.height }, ref: this.mapRef, attributionControl: false, crs: CRS.Simple },
            this.props.showScaleY && (React.createElement(VerticalZoom, { position: "topleft", scaleY: this.props.scaleY, minScaleY: 1, maxScaleY: 10 })),
            React.createElement(ScaleControl, { position: "bottomright", imperial: false, metric: true }),
            React.createElement(OptionalLayerControl, { showLayersControl: showLayersControl },
                this.props.layers
                    .filter((layer) => layer.base_layer)
                    .map((layer) => showLayersControl ? (React.createElement(BaseLayer, { checked: layer.checked, name: layer.name, key: layer.name }, renderBaseLayer(layer))) : (renderBaseLayer(layer, layer.name))),
                this.props.layers
                    .filter((layer) => !layer.base_layer)
                    .map((layer) => showLayersControl ? (React.createElement(Overlay, { checked: layer.checked, name: layer.name, key: layer.name }, renderOverlay(layer))) : (renderOverlay(layer, layer.name)))),
            showDrawControls && (React.createElement(FeatureGroup, null,
                React.createElement(DrawControls, { drawMarker: draw_toolbar_marker, drawPolygon: draw_toolbar_polygon, drawPolyline: draw_toolbar_polyline, lineCoords: (coords) => setProps({ polyline_points: coords }), markerCoords: (coords) => setProps({ marker_point: coords }), polygonCoords: (coords) => setProps({ polygon_points: coords }) }))),
            showHillshadingSwitch && (React.createElement(Switch, { position: "bottomleft", label: "Hillshading", checked: this.props.hillShading, onChange: this.handleHillshadingChange.bind(this) })),
            this.state.x !== null && (React.createElement(Fragment, null,
                React.createElement(CircleMarker, { center: [this.state.y, this.state.x], color: "red", radius: 5 }),
                React.createElement(ValueInfoBox, { position: "bottomleft", x: this.state.x, y: this.state.y, z: this.state.z })))));
    }
}
...
*     }
*/
//# sourceMappingURL=LayeredMap.js.map