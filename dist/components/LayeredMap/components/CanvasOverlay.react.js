...
class CanvasOverlay extends MapLayer {
    ...
    _animateZoom(e) {
        L.DomUtil.setTransform(this.el, this._map._latLngBoundsToNewLayerBounds(this._bounds, e.zoom, e.center).min, this._map.getZoomScale(e.zoom));
    }
    _reset() {
    ...
    componentDidMount() {
        ...
        this.el.onclick = (e) => {
            if (this.props.original_data.loaded) {
                ...
                const x = Math.floor(((e.clientX - client_rect.left) / client_rect.width) *
                    this.props.original_data.ImageData.width);
                const y = Math.floor(((e.clientY - client_rect.top) / client_rect.height) *
                    this.props.original_data.ImageData.height);
                // RGBA
                const NUMBER_COLOR_CHANNELS = 4;
                const NUMBER_DISCRETIZATION_LEVELS = 255;
                const z = this.props.original_data.ImageData.data[(y * this.props.original_data.ImageData.width + x) *
                    NUMBER_COLOR_CHANNELS];
                const z_string = z > 0
                    ? `${Math.floor(((this.props.maxvalue - this.props.minvalue) *
                        (z - 1)) /
...
//# sourceMappingURL=CanvasOverlay.react.js.map