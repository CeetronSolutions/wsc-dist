...
const MousePosition = (props) => {
    ...
    const focusedDependencyArray = () => {
        ...
        const newURL = focusedImageLayer && focusedImageLayer.getUrl
            ? focusedImageLayer.getUrl()
            : null;
        const newCanvas = focusedImageLayer && focusedImageLayer.getCanvas
    ...
    const updateStateCanvas = (canvas, onScreenCanvas, ctx, minvalue, maxvalue, unit) => {
        stateRef.current = Object.assign(Object.assign({}, stateRef.current), { canvas,
            ctx,
            onScreenCanvas,
            minvalue,
            maxvalue,
    ...
    const updateCanvas = () => __awaiter(void 0, void 0, void 0, function* () {
         ...
         * @type {Image}
         */
        const image = yield new Promise((res) => {
    ...
    const onCanvasMouseMove = (event) => {
        ...
        const screenX = ((event.originalEvent.clientX - clientRect.left) /
            clientRect.width) *
            canvas.width;
        const screenY = ((event.originalEvent.clientY - clientRect.top) /
            clientRect.height) *
            canvas.height;
    ...
    const onCanvasMouseClick = (event) => {
        ...
        for (const control of controlsList) {
            ...
            const forbiddenYValueRange = [
                ...
                mapYCoordinateToCanvas(controlBounds.y + controlBounds.height, controlBounds, canvas),
            ];
            const forbiddenXValueRange = [
                mapXCoordinateToCanvas(controlBounds.x, controlBounds, canvas),
                mapXCoordinateToCanvas(controlBounds.x + controlBounds.width, controlBounds, canvas),
            ];
        ...
        if (props.setProps &&
            ...
            !isForbidden(event.originalEvent.clientX, event.originalEvent.clientY, forbiddenCoordinates)) {
    ...
    const mapZValue = (r, g, b) => {
        ...
        const scaleFactor = (maxvalue - minvalue) / (256.0 * 256.0 * 256.0 - 1.0);
