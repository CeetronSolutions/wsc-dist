...
import { bindBuffer, bindTexture, createProgram, createShader, } from "./webglutils";
...
export const drawCommand = (context, cmd, props = {}) => {
    ...
    // Add the textures
    Object.entries(cmd.textures).forEach(([textureName, { textureUnit, textureImage }]) => {
    ...
    // Add uniforms
    Object.entries(cmd.uniforms).forEach(([uniformName, { value, type = "1f" }]) => {
        ...
        if (gl[uniformFuncName] instanceof Function) {
            gl[uniformFuncName](uniformLocation, ...(Array.isArray(uniformValue)
                ? uniformValue
                : [uniformValue]));
        }
        else {
