...
const getIndexies = (layers) => {
    ...
    layers.forEach((kLayer) => {
        kLayer.forEach((_a) => {
            var { k, i, j } = _a, layer = __rest(_a, ["k", "i", "j"]);
            if (!index[k]) {
                index[k] = {};
            }
            if (!index[k][i]) {
...
const addNegativeFlow = ({ layers, indexies }) => layers.map((kLayer) => kLayer.map((_a) => {
    var { i, j, k } = _a, layer = __rest(_a, ["i", "j", "k"]);
    let FLOWInegative = 0;
    let FLOWJnegative = 0;
    if (indexies[k][i - 1] &&
        indexies[k][i - 1][j] &&
    ...
    return Object.assign(Object.assign({}, layer), { k,
        i,
        j, "FLOWI-": FLOWInegative, "FLOWJ-": FLOWJnegative });
}));
...
const initFlowMap = ({ canvasSelector, elementSelector, data, height, layerNames, }) => {
...
class Map extends Component {
    ...
    render() {
        ...
        return (React.createElement("div", { style: {
            ...
            React.createElement("div", { id: this.elementId },
                React.createElement("canvas", { id: this.canvasId, ref: (ref) => {
                        this.canvas = ref;
                    }, style: {
                        pointerEvents: "none",
                        position: "absolute",
