...
export default class FlowMap extends Map2D {
    constructor(_a) {
        var { canvasSelector, layers } = _a, rest = __rest(_a, ["canvasSelector", "layers"]);
        ...
        this._setLayer(0);
        this._flowAnimation = new FlowAnimation(this._canvasNode, 0, 1, this._particleGenerator, 1500, this.kInit);
    }
    _setNormalizedFlux() {
        const self = this;
    ...
    _createCells(i) {
        ...
        this.layers[i].forEach((cell) => {
            cells.push(new Cell(cell.points.map(([x, y]) => [x - self.xMin, self.yMax - y]), cell.i, cell.j, cell["NORMFLOWI-"] || cell["FLOWI-"], cell["NORMFLOWJ-"] || cell["FLOWJ-"], cell["NORMFLOWI+"] || cell["FLOWI+"], cell["NORMFLOWJ+"] || cell["FLOWJ+"]));
        });
        return cells;
    }
    _setLayer(i) {
    ...
    init() {
        ...
        this.on("zoom", (t) => {
            ...
            self._flowAnimation.setTransform(t.x, t.y, t.k, this.kInit, t.angle, [(t.k * this.map.mapWidth) / 2, (t.k * this.map.mapHeight) / 2]);
        });
        this.on("rotate", (t) => {
            self._flowAnimation.clear();
            self._flowAnimation.setTransform(t.x, t.y, t.k, this.kInit, t.angle, [(t.k * this.map.mapWidth) / 2, (t.k * this.map.mapHeight) / 2]);
    ...
    initResize() {
        ...
        const resizeCanvas = () => {
            ...
            this._flowAnimation.setTransform(t.x, t.y, t.k, this.kInit, t.angle, [(t.k * this.map.mapWidth) / 2, (t.k * this.map.mapHeight) / 2]);
        };
