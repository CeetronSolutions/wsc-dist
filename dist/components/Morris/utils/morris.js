    ...
    const proto = {
        ...
        initGraphSVG() {
            ...
            this._graphG = this._graphSVG
                .append("g")
                .attr("transform", `translate(${this._graphContainerMargin.left}, ${this._graphContainerMargin.top})`);
            return this;
        },
        updateGraphSVG() {
            this._graphSVG.attr("width", this._graphWidth +
                this._graphContainerMargin.left +
                this._graphContainerMargin.right);
            return this;
        },
        initParameterDomains() {
            ...
            this._parameterDomains = {
                main: {
                    ...
                    log: d3.extent(allMainValues
                        .filter((value) => value !== 0)
                        .map(Math.log)),
                },
                interaction: {
                    linear: d3.extent(allInteractionValues),
                    log: d3.extent(allInteractionValues
                        .filter((value) => value !== 0)
                        .map(Math.log)),
                },
            };
        ...
        updateSliderPositions() {
            this._sliderCellXPos = this._sliderColumns.map((d, j) => this._sliderCellRelativeXPos[j] * this._sliderRowWidth, this);
        ...
        updateSliderCellWidths() {
            this._sliderCellWidths = this._sliderColumns.map((d, j) => this._sliderCellRelativeWidths[j] * this._sliderRowWidth, this);
            return this;
        },
        initSliderWidths() {
        ...
        updateSliderWidths() {
            this._sliderWidths = this._sliderColumns.map((d, j) => this._sliderCellRelativeWidths[j] * this._sliderRowWidth -
                this._sliderCellPadding.left -
                this._sliderCellPadding.right, this);
            return this;
        },
        initSliderRanges() {
        ...
        updateSliderRanges() {
            this._sliderXRanges = this._sliderColumns.map((d, j) => [0, this._sliderWidths[j]], this);
            return this;
        },
        initSliderScale() {
            this._mainX = "linear";
        ...
        updateSliderBars() {
            ...
            const updateSliderBars = (selector, type, scaleType) => {
                ...
                function drawBackgroundBar() {
                    ...
                    const rect = cell
                        .selectAll(".sensitivity-slider-plot__slider-bar-background")
                        .data((d) => [d]);
                    rect.enter()
                        .append("rect")
                ...
                function drawInteractionBar(d) {
                    ...
                    const x = getScale(self._parameterDomains[d.type][scaleType], self._sliderXRanges[d.j], scaleType);
                    const rect = cell
                        .selectAll(".sensitivity-slider-plot__slider-bar-interaction")
                        .data((d) => [d]);
                    rect.enter()
                        .append("rect")
                ...
                function drawValueLabel(d) {
                    ...
                    text.enter()
                        ...
                        const val = d.values[scaleType][self._currentGraphDatumIndex];
                        if (Number.isFinite(val)) {
                            return val.toPrecision(self._labelNumericPrecision);
                        }
                        return "";
                    })
                        .attr("x", (d, i, nodes) => {
        ...
        createOnMouseMove() {
            ...
            function onMouseMove() {
                ...
                const labelX = invertLabelPosition
                    ...
                    : GRAPH_FOCUS_X;
                const maxPosition = self._graphY(d.min) - self._graphY(d.max) < 30
                    ? self._graphY(d.max) - 30
                    : self._graphY(d.max);
                const meanPosition = self._graphY(d.min) - self._graphY(d.max) < 30
                    ? self._graphY(d.mean) - 15
                    : self._graphY(d.mean);
                self._graphFocus.attr("transform", `translate(${self._graphX(d.time)}, 0)`);
                self._graphFocus
                    .select("circle.max")
