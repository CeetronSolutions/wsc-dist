...
import { DrawLineStringMode, DrawPointMode, DrawPolygonMode, ImmutableFeatureCollection, ModifyMode, TransformMode, ViewMode, } from "@nebula.gl/edit-modes";
...
function deleteEscapeKeyHandler(drawMode, event, props) {
    if (event.key === "Escape")
        drawMode.getClickSequence().pop();
...
const UNSELECTED_LINE_COLOR = [0x50, 0x50, 0x50, 0xcc];
const SELECTED_LINE_COLOR = [0x0, 0x0, 0x0, 0xff];
const defaultProps = {
    "@@type": "DrawingLayer",
    name: "Drawing",
    id: "drawing-layer",
...
export default class DrawingLayer extends CompositeLayer {
    ...
    getPickingInfo({ info }) {
        ...
        if (feature.geometry.type === "LineString") {
            ...
            return info;
        return Object.assign(Object.assign({}, info), { propertyValue: measurement });
    }
        ...
        switch (editAction.editType) {
            case "addFeature":
                this.setState({
                    ...
                    selectedFeatureIndexes: editAction.editContext.featureIndexes,
                });
                this.context.userData.setEditedData({
                    data: editAction.updatedData,
                    selectedFeatureIndexes: editAction.editContext.featureIndexes,
    ...
    _getLineColor(feature) {
        const is_feature_selected = this.state["selectedFeatureIndexes"].some((i) => this.state["data"].features[i] === feature);
    ...
    renderLayers() {
        ...
        const sub_layer_props = this.getSubLayerProps({
            ...
            _subLayerProps: {
                geojson: {
                    autoHighlight: true,
                    getLineColor: (feature) => this._getLineColor(feature),
                },
            },
        });
