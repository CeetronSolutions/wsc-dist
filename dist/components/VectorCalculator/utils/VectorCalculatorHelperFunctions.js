...
export const isValidExpressionNameString = (name) => {
    const regex = new RegExp(/^(?=.{1,50}$)[A-Za-z]{1}([:_]?[A-Za-z0-9]+){0,}$/);
    return regex.test(name);
};
export const isNameOccupiedByVectors = (name, vectors) => {
    const nodes = name.split(":");
    let children = vectors;
    for (const node of nodes) {
        const childNode = children.find((child) => child.name === node);
...
export const isExpressionNameValidAndNotOccupiedByVectors = (name, vectors) => {
    return (isValidExpressionNameString(name) &&
        !isNameOccupiedByVectors(name, vectors));
};
export const expressionNameValidationMessage = (name) => {
...
export const doesNameExistInExpressionList = (name, expressions) => {
    return expressions.some((expression) => expression.name === name);
};
export const getDetailedExpression = (expression) => {
    ...
    const expressionSplit = expression.expression.split(/(?=[^a-zA-Z])|(?<=[^a-zA-Z])/g);
...
const getVariablesVectorMap = (variableVectorMap) => {
    const map = new Map();
    variableVectorMap.forEach((elm) => map.set(elm.variableName, elm.vectorName[0]));
    return map;
};
export const getAvailableName = (nameSuggestion, expressions) => {
...
export const isVariableVectorMapValid = (variableVectorMap, delimiter, vectorData) => {
    if (variableVectorMap.length <= 0) {
        return false;
    }
    const vectorNames = variableVectorMap.map((pair) => {
...
export const areVariableVectorMapsEqual = (first, second) => {
    if (first.length !== second.length) {
        return false;
    }
    // Compare if the element matches in the same index
    const res = first.every((elm, idx) => {
        return (elm.variableName === second[idx].variableName &&
            elm.vectorName[0] === second[idx].vectorName[0]);
    });
    return res;
};
export const areAllVectorNamesValid = (names, delimiter, vectorData) => {
    return names.every((name) => name === undefined
        ? false
        : isVectorNameValid(name, delimiter, vectorData));
};
export const isVectorNameValid = (name, delimiter, vectorData) => {
    const nodePath = name.split(delimiter);
    ...
    const metaNodes = treeData.findFirstNode(nodePath, true);
    return metaNodes !== null;
};
export const createVariableVectorMapFromVariables = (variables, variableVectorMap) => {
    return variables.map((variable) => {
        const mapElm = variableVectorMap.find((elm) => elm.variableName === variable);
        if (!mapElm) {
...
//# sourceMappingURL=VectorCalculatorHelperFunctions.js.map