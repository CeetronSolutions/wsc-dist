...
*/
export function colorToString(color, // [r,g,b]
cDefault) {
    if (!color)
        return cDefault;
    const p = 0x1000000 | // force value to have 7 hex digits : 1rrggbb
        (color[0] << 16) |
        (color[1] << 8) |
        color[2];
...
*/
export function color4ToString(color // [stop, r,g,b]
) {
    // see colorToString()
...
*/
export function getInterpolatedColor(colorTable, v) {
    // TODO: Do not compute these 3 constants (cNaN, cBelow, cAbove) every time!
    const cNaN = colorTable.colorNaN
        ? colorTable.colorNaN
        : [255, 255, 255]; // "white"
    if (Number.isNaN(v)) {
        return cNaN;
...
/*
  get HTML string with interpolated color value in #xxxxxx format
*/
export function getInterpolatedColorString(colorTable, v) {
    // TODO: Do not compute these 3 constants (cNaN, cBelow, cAbove) every time!
...
//# sourceMappingURL=color-table.js.map