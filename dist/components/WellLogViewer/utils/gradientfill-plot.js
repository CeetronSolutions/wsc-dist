 ...
 */
function createGradient(ctx, yscale, horizontal, plotdata, xscale, colorTable, scale // "linear" | "log"
) {
...
export default class GradientFillPlot extends Plot {
    constructor(id, options = {}) {
        ...
        this.options.renderLegend = renderGradientFillPlotLegend; // see updateLegendRows() in videx-wellog\src\tracks\graph\graph-legend.ts 
    }
    /**
    ...
    plot(ctx, scale) {
        ...
        if (options.inverseColor || options.inverseColorTable) {
            ...
            if (colorTable)
                ctx.fillStyle = createGradient(ctx, scale, options.horizontal, plotdata, xscale, colorTable, options.inverseColorScale ||
                    options.colorScale ||
                    options.scale);
        ...
        if (colorTable)
            ctx.fillStyle = createGradient(ctx, scale, options.horizontal, plotdata, xscale, colorTable, options.colorScale || options.scale);
        /* End GradientFill code */
        ctx.fill();
        ctx.globalAlpha = 1;
...
//# sourceMappingURL=gradientfill-plot.js.map